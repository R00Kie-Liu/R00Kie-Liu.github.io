...
> **本节重点：** 预训练的关键变量不再只是 token 数，而是数据在什么阶段出现、是否可验证、是否能构成长上下文、代码仓库或 agent 轨迹。
...
> **本节重点：** Mid-training 正在承担扩长、能力迁移和架构适配的中间层角色，是连接 base pretraining 与 SFT/RL 的关键阶段。
...
> **本节重点：** 后训练已经从“让模型更会聊天”扩展为任务环境驱动的能力生产线，真正的瓶颈往往在 verifier、rollout、sandbox 和调度系统。
...
> **本节重点：** Optimizer、LR schedule、辅助目标和能力整合方式已经成为显性 recipe；它们决定模型能否稳定训练到目标能力，而不只是影响收敛速度。
...
> **本节重点：** MoE 的竞争已经从“更多专家、更低 active params”转向可训练、可均衡、可通信、可服务的稀疏计算系统。
...
> **本节重点：** 长上下文的竞争不再是窗口长度的 headline，而是模型能否以可承受的 KV/cache、prefill/decode 和 rollout 成本持续使用长上下文。
...
> **本节重点：** 多模态正在从输入扩展变成 agent 的感知与行动层；真正重要的是模型能否把图像、音频、视频和 GUI 状态转化为可靠行动。
...
> **研究视角：** 这一章的重点不是给出最终定论，而是把 think/agentic、SFT、扩长和数据工程这些问题拆成可继续追问的研究假设。
...
> **六个问题的共同结论：** think、agentic、SFT、扩长和数据工程都不适合用单一标签解释，更适合放回 pretraining、mid-training、post-training、RL 和 serving 的链条中理解。
...
> **本节重点：** Open weights 不等于 fully open；能力可用和训练可复现是两种不同层次的开放。
...
> **本节重点：** 公开报告的最大盲区仍是数据配比、后训练规模、base agentic eval、packing 后真实 token mixture 和 dataloader 消费顺序。
...
> **总判断：** 2025-2026 年的开放大模型已经进入系统协同阶段，模型能力不再由单一因素驱动，而是由 MoE、长上下文、数据工程、后训练、RL 和 serving 共同决定。
